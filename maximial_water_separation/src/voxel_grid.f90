...
! separation and the closet atom enumeration and the enumerate atoms image
    ...
    INTEGER(4)  ::  i,j,k, vPoint(3),target_imageV(3),direction, furthest_imageV(3)
...
& .AND. voxel_grid(i,j,k)%target_separation > r_hold) THEN
      ...
      END DO
    END DO
    furthest_imageV=voxel_grid(vPoint(1),vPoint(2),vPoint(3))%closest_image
    !PRINT *, "furthest image : ", furthest_imageV
    !PRINT *, "furthest atom ", voxel_grid(vPoint(1),vPoint(2),vPoint(3))%closest_atom
    target_imageV=voxel_grid(vPoint(1),vPoint(2),vPoint(3))%target_image
    element=crystal(furthest_imageV(1)+2,furthest_imageV(2)+2,furthest_imageV(3)+2, &
