...
!        DO j=1,3
!          
! note we do need to do an overall check
  ...
  FUNCTION crys_separation(crystal,targetN, water)
    TYPE(atom), DIMENSION(:,:,:,:), INTENT(IN) ::  crystal
    INTEGER(8), INTENT(IN)  ::  targetN
    TYPE(atom), INTENT(IN)  ::  water
    REAL(8) ::  crys_separation
    INTEGER(2)  :: l,j,k,i
    TYPE(atom)  ::  target_atom
    REAL(8) ::  separation
    crys_separation=1000
    DO l=1,3
     DO k=1,3
      DO j=1,3
        target_atom=crystal(j,k,l,targetN)
        separation=0
        DO i=1,3
          separation=separation+(water%cartesian(i)-target_atom%cartesian(i))**2
        END DO
        separation = SQRT(separation)
        IF(separation<crys_separation) THEN
          crys_separation=separation
        END IF
      END DO
     END DO
    END DO
  END FUNCTION crys_separation
